<section class="answer">
    <div class="container-fluid">
        <div class="row">
            <div class="col-12 answer__block">
                <img src="./assets/img/answer/no.svg" alt="Иконка">
                <h1>Thank You!</h1>
                <h2>Your Payment is Successfull</h2>
                <p>You will receive an email with confirmation details. If you have any questions regarding your
                    payment, please send a request through the <a class="scrolling" href="#order-id"><i>form below</i></a></p>
                <a href="#" class="button button_blue">Back to home</a>
            </div>
        </div>
    </div>
</section>